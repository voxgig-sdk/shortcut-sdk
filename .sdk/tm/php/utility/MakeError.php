...
class ShortcutMakeError
{
    public static function call(?ShortcutContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ShortcutContext([], null);
        }
        $op = $ctx->op ?? new ShortcutOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ShortcutResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ShortcutError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ShortcutError('', $msg, $ctx);
