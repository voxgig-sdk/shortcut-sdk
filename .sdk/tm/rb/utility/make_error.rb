...
module ShortcutUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ShortcutContext.new({}, nil)
    end
    op = ctx.op || ShortcutOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ShortcutResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ShortcutError) ? err.msg : err.to_s
    ...
    sdk_err = ShortcutError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ShortcutError)
