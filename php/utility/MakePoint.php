...
// Shortcut SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class ShortcutMakePoint
{
    public static function call(ShortcutContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof ShortcutError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = ShortcutHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = ShortcutHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
