...
class EpicEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ShortcutSDK::test(null, null);
        $ent = $testsdk->Epic(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Epic(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Epic(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = epic_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "epic." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_EPIC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $epic_ref01_ent = $client->Epic(null);
        ...
        $epic_ref01_list_result = $epic_ref01_ent->list($epic_ref01_match, null);
        $this->assertIsArray($epic_ref01_list_result);
        ...
        $epic_ref01_markdef_up0_value = "Mark01-epic_ref01_" . $setup["now"];
        ...
        $epic_ref01_ent->remove($epic_ref01_match_rm0, null);
        ...
        $epic_ref01_list_rt0_result = $epic_ref01_ent->list($epic_ref01_match_rt0, null);
        $this->assertIsArray($epic_ref01_list_rt0_result);
...
function epic_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/epic/EpicTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["epic01", "epic02", "epic03", "label01", "label02", "label03", "milestone01", "milestone02", "milestone03", "objectif01", "objectif02", "objectif03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_EPIC_ENTID");
