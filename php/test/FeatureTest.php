...
class FtCtrl extends ShortcutControl
{
...
class FtClient
{
    ...
    private ShortcutUtility $utility;

    public function __construct(ShortcutUtility $utility, array $options)
    {
    ...
    public function get_utility(): ShortcutUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ShortcutContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ShortcutContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ShortcutContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ShortcutConfig::make_feature($name);
    ...
    private static function buildUrl(ShortcutSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ShortcutError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ShortcutResponse($fetched);
        }
    ...
    private function fail(ShortcutContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ShortcutContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
