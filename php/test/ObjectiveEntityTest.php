...
class ObjectiveEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ShortcutSDK::test(null, null);
        $ent = $testsdk->Objective(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Objective(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Objective(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = objective_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "objective." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_OBJECTIVE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $objective_ref01_ent = $client->Objective(null);
        ...
        $objective_ref01_list_result = $objective_ref01_ent->list($objective_ref01_match, null);
        $this->assertIsArray($objective_ref01_list_result);
        ...
        $objective_ref01_markdef_up0_value = "Mark01-objective_ref01_" . $setup["now"];
...
function objective_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/objective/ObjectiveTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["objective01", "objective02", "objective03", "objectif01", "objectif02", "objectif03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_OBJECTIVE_ENTID");
