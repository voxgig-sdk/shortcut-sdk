...
class CustomFieldEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_CUSTOM_FIELD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $custom_field_ref01_data = null;
        if (count($custom_field_ref01_data_raw) > 0) {
...
function custom_field_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["custom_field01", "custom_field02", "custom_field03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_CUSTOM_FIELD_ENTID");
