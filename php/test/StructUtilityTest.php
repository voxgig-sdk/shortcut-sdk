...
// Shortcut SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ShortcutOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ShortcutOmni::structfix($now, Struct::undef()))
                        !== json_encode(ShortcutOmni::structfix($val, Struct::undef()))) {
