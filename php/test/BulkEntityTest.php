...
class BulkEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_BULK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function bulk_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["bulk01", "bulk02", "bulk03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_BULK_ENTID");
