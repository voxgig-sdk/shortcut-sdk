...
class WorkflowEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Workflow(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Workflow(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_WORKFLOW_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $workflow_ref01_list_result = $workflow_ref01_ent->list($workflow_ref01_match, null);
        $this->assertIsArray($workflow_ref01_list_result);
...
function workflow_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_WORKFLOW_ENTID");
