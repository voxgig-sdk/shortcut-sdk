...
class LinkedFileEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ShortcutSDK::test(null, null);
        $ent = $testsdk->LinkedFile(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->LinkedFile(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->LinkedFile(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = linked_file_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "linked_file." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_LINKED_FILE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $linked_file_ref01_ent = $client->LinkedFile(null);
        ...
        $linked_file_ref01_list_result = $linked_file_ref01_ent->list($linked_file_ref01_match, null);
        $this->assertIsArray($linked_file_ref01_list_result);
        ...
        $linked_file_ref01_ent->remove($linked_file_ref01_match_rm0, null);

        // LIST
        $linked_file_ref01_match_rt0 = [];

        $linked_file_ref01_list_rt0_result = $linked_file_ref01_ent->list($linked_file_ref01_match_rt0, null);
        $this->assertIsArray($linked_file_ref01_list_rt0_result);
...
function linked_file_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/linked_file/LinkedFileTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["linked_file01", "linked_file02", "linked_file03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_LINKED_FILE_ENTID");
