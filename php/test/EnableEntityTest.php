...
class EnableEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_ENABLE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function enable_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_ENABLE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["SHORTCUT_TEST_ENABLE_ENTID"]);
