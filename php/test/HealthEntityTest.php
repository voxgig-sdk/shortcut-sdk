...
class HealthEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_HEALTH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function health_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["health01", "health02", "health03", "epic01", "epic02", "epic03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_HEALTH_ENTID");
