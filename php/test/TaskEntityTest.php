...
class TaskEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_TASK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function task_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["task01", "task02", "task03", "story01", "story02", "story03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_TASK_ENTID");
