...
class StorySlimEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ShortcutSDK::test(null, null);
        $ent = $testsdk->StorySlim(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = story_slim_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "update"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "story_slim." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_STORY_SLIM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $story_slim_ref01_ent = $client->StorySlim(null);
...
function story_slim_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/story_slim/StorySlimTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["story_slim01", "story_slim02", "story_slim03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_STORY_SLIM_ENTID");
