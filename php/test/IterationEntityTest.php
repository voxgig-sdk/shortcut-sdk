...
class IterationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ShortcutSDK::test(null, null);
        $ent = $testsdk->Iteration(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Iteration(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Iteration(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = iteration_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list", "update", "load", "remove"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "iteration." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_ITERATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $iteration_ref01_ent = $client->Iteration(null);
        ...
        // LIST
        $iteration_ref01_match = [];

        $iteration_ref01_list_result = $iteration_ref01_ent->list($iteration_ref01_match, null);
        $this->assertIsArray($iteration_ref01_list_result);
        ...
        // UPDATE
        $iteration_ref01_data_up0_up = [
            "id" => $iteration_ref01_data["id"],
        ];

        $iteration_ref01_markdef_up0_name = "app_url";
        $iteration_ref01_markdef_up0_value = "Mark01-iteration_ref01_" . $setup["now"];
        ...
        // REMOVE
        $iteration_ref01_match_rm0 = [
            "id" => $iteration_ref01_data["id"],
        ];
        $iteration_ref01_ent->remove($iteration_ref01_match_rm0, null);

        // LIST
        $iteration_ref01_match_rt0 = [];

        $iteration_ref01_list_rt0_result = $iteration_ref01_ent->list($iteration_ref01_match_rt0, null);
        $this->assertIsArray($iteration_ref01_list_rt0_result);
...
function iteration_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/iteration/IterationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["iteration01", "iteration02", "iteration03"] as $k) {
    ...
    $entid_env_raw = getenv("SHORTCUT_TEST_ITERATION_ENTID");
