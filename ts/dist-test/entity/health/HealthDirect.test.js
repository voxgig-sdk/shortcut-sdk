    ...
    (0, node_test_1.test)('direct-load-health', async (t) => {
        ...
        if (setup.live) {
            const listResult = await client.direct({
                path: 'api/v3/epics/{epic_id}/health-history',
                method: 'GET',
                params: {
            ...
            const candidateId = listArr[0]?.epic_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.epic_id = candidateId;
        ...
        const result = await client.direct({
            path: 'api/v3/epics/{epic_id}/health',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['SHORTCUT_TEST_HEALTH_ENTID'];
