...
(0, node_test_1.describe)('ObjectiveDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-objective', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-objective', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["objective_public01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.objective_public_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.objective_public_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-objective', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-objective', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ShortcutSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.SHORTCUT_APIKEY,
        }));
        let idmap = env['SHORTCUT_TEST_OBJECTIVE_ENTID'];
    ...
    const client = new __1.ShortcutSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ObjectiveDirect.test.js.map