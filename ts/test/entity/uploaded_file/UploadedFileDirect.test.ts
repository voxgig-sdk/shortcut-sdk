...
describe('UploadedFileDirect', async () => {
  ...
  test('direct-load-uploaded_file', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-uploaded_file', setup.live)) return
  ...
  test('direct-list-uploaded_file', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-uploaded_file', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHORTCUT_TEST_UPLOADED_FILE_ENTID']
