...
describe('LinkedFileDirect', async () => {
  ...
  test('direct-load-linked_file', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-linked_file', setup.live)) return
  ...
  test('direct-list-linked_file', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-linked_file', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHORTCUT_TEST_LINKED_FILE_ENTID']
