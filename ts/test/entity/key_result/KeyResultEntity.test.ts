...
describe('KeyResultEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'key_result.' + op, live)) return
    }
    ...
    let key_result_ref01_data = Object.values(setup.data.existing.key_result)[0] as any

    // UPDATE
    const key_result_ref01_ent = client.KeyResult()
    const key_result_ref01_data_up0: any = {}
    ...
    ;(key_result_ref01_data_up0 as any)[key_result_ref01_markdef_up0.name] = key_result_ref01_markdef_up0.value
    ...
    assert((key_result_ref01_resdata_up0 as any)[key_result_ref01_markdef_up0.name] === key_result_ref01_markdef_up0.value)


    // LOAD
    const key_result_ref01_match_dt0: any = {}
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['SHORTCUT_TEST_KEY_RESULT_ENTID']
