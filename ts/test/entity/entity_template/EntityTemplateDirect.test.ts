...
describe('EntityTemplateDirect', async () => {
  ...
  test('direct-load-entity_template', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-entity_template', setup.live)) return
  ...
  test('direct-list-entity_template', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-entity_template', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHORTCUT_TEST_ENTITY_TEMPLATE_ENTID']
