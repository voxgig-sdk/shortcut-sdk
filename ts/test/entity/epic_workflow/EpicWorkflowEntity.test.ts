...
describe('EpicWorkflowEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'epic_workflow.' + op, live)) return
    }
    ...
    let epic_workflow_ref01_data = Object.values(setup.data.existing.epic_workflow)[0] as any

    // LIST
    const epic_workflow_ref01_ent = client.EpicWorkflow()
    const epic_workflow_ref01_match: any = {}

    const epic_workflow_ref01_list = (await epic_workflow_ref01_ent.list(epic_workflow_ref01_match)).map((e: any) => e.data())
