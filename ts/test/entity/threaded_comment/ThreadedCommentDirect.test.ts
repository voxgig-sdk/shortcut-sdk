...
describe('ThreadedCommentDirect', async () => {
  ...
  test('direct-load-threaded_comment', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-threaded_comment', setup.live)) return
    if (skipIfMissingIds(t, setup, ["epic01"])) return
  ...
  test('direct-list-threaded_comment', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-threaded_comment', setup.live)) return
    if (skipIfMissingIds(t, setup, ["epic01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['SHORTCUT_TEST_THREADED_COMMENT_ENTID']
