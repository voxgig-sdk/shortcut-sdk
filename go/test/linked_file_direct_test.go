...
func TestLinkedFileDirect(t *testing.T) {
	t.Run("direct-list-linked_file", func(t *testing.T) {
		setup := linked_fileDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-linked_file", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-linked_file", func(t *testing.T) {
		setup := linked_fileDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-linked_file", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type linked_fileDirectSetupResult struct {
...
func linked_fileDirectSetup(mockres any) *linked_fileDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"SHORTCUT_TEST_LINKED_FILE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["SHORTCUT_TEST_LINKED_FILE_ENTID"]; ok {
		...
		return &linked_fileDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &linked_fileDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
