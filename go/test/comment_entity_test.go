...
func TestCommentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_COMMENT_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		commentRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.comment")))
		var commentRef01Data map[string]any
		if len(commentRef01DataRaw) > 0 {
			commentRef01Data = core.ToMapAny(commentRef01DataRaw[0][1])
		}
...
func commentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"comment01", "comment02", "comment03", "story01", "story02", "story03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("SHORTCUT_TEST_COMMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SHORTCUT_TEST_COMMENT_ENTID"])
