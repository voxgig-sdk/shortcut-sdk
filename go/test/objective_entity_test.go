...
func TestObjectiveEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		objectiveRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "objective"}, setup.data), "objective_ref01"))

		objectiveRef01DataResult, err := objectiveRef01Ent.Create(objectiveRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		objectiveRef01Data = core.ToMapAny(objectiveRef01DataResult)
		...
		objectiveRef01ResdataUp0 := core.ToMapAny(objectiveRef01ResdataUp0Result)
		...
		objectiveRef01DataDt0LoadResult := core.ToMapAny(objectiveRef01DataDt0Loaded)
