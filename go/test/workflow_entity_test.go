...
func TestWorkflowEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_WORKFLOW_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		workflowRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.workflow", setup.data)))
		...
		_, workflowRef01ListOk := workflowRef01ListResult.([]any)
		...
		workflowRef01DataDt0LoadResult := core.ToMapAny(workflowRef01DataDt0Loaded)
...
func workflowBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SHORTCUT_TEST_WORKFLOW_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SHORTCUT_TEST_WORKFLOW_ENTID"])
