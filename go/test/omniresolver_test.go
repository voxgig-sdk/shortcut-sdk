...
import (
	...
	omni "github.com/voxgig-sdk/shortcut-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ShortcutError); is {
				out := map[string]any{
					"name":    "ShortcutError",
...
func omniCtx(arg any, client *sdk.ShortcutSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ShortcutSDK); is {
			client = live
