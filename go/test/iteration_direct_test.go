...
func TestIterationDirect(t *testing.T) {
	t.Run("direct-list-iteration", func(t *testing.T) {
		setup := iterationDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-iteration", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-iteration", func(t *testing.T) {
		setup := iterationDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-iteration", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type iterationDirectSetupResult struct {
...
func iterationDirectSetup(mockres any) *iterationDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"SHORTCUT_TEST_ITERATION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["SHORTCUT_TEST_ITERATION_ENTID"]; ok {
		...
		return &iterationDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &iterationDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
