...
func TestMemberEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_MEMBER_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		memberRef01DataDt0LoadResult := core.ToMapAny(memberRef01DataDt0Loaded)
...
func memberBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("SHORTCUT_TEST_MEMBER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["SHORTCUT_TEST_MEMBER_ENTID"])
