...
func TestObjectifEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.Objectif(nil)
		if ent == nil {
			t.Fatal("expected non-nil ObjectifEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := objectifBasicSetup(nil)
		...
		for _, _op := range []string{} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "objectif." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_OBJECTIF_ENTID JSON to run live")
			return
		}
...
func objectifBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "objectif", "ObjectifTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read objectif test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse objectif test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("SHORTCUT_TEST_OBJECTIF_ENTID")
