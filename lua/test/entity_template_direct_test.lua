...
describe("EntityTemplateDirect", function()
  it("should direct-list-entity_template", function()
    local setup = entity_template_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-entity_template", setup.live and "live" or "unit")
  ...
  it("should direct-load-entity_template", function()
    local setup = entity_template_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-entity_template", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function entity_template_direct_setup(mockres)
