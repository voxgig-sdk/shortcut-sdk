...
describe("ThreadedCommentDirect", function()
  it("should direct-list-threaded_comment", function()
    local setup = threaded_comment_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-threaded_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"epic01"}) do
  ...
  it("should direct-load-threaded_comment", function()
    local setup = threaded_comment_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-threaded_comment", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function threaded_comment_direct_setup(mockres)
