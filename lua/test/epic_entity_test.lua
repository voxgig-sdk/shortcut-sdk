...
describe("EpicEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Epic(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Epic(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Epic(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = epic_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list", "update", "load", "remove"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "epic." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_EPIC_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local epic_ref01_ent = client:Epic(nil)
    local epic_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local epic_ref01_match = {
      ["objectif_id"] = setup.idmap["objectif01"],
    }

    local epic_ref01_list_result, err = epic_ref01_ent:list(epic_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(epic_ref01_list_result)
    ...
    -- UPDATE
    local epic_ref01_data_up0_up = {
      id = epic_ref01_data["id"],
    }

    local epic_ref01_markdef_up0_name = "app_url"
    local epic_ref01_markdef_up0_value = "Mark01-epic_ref01_" .. tostring(setup.now)
    ...
    -- REMOVE
    local epic_ref01_match_rm0 = {
      id = epic_ref01_data["id"],
    }
    local _, err = epic_ref01_ent:remove(epic_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local epic_ref01_match_rt0 = {
      ["objectif_id"] = setup.idmap["objectif01"],
    }

    local epic_ref01_list_rt0_result, err = epic_ref01_ent:list(epic_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(epic_ref01_list_rt0_result)
...
function epic_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/epic/EpicTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read epic test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("SHORTCUT_TEST_EPIC_ENTID")
