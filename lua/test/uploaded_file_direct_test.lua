...
describe("UploadedFileDirect", function()
  it("should direct-list-uploaded_file", function()
    local setup = uploaded_file_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-uploaded_file", setup.live and "live" or "unit")
  ...
  it("should direct-load-uploaded_file", function()
    local setup = uploaded_file_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-uploaded_file", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function uploaded_file_direct_setup(mockres)
