...
describe("IterationDirect", function()
  it("should direct-list-iteration", function()
    local setup = iteration_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-iteration", setup.live and "live" or "unit")
  ...
  it("should direct-load-iteration", function()
    local setup = iteration_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-iteration", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function iteration_direct_setup(mockres)
