...
describe("LinkedFileDirect", function()
  it("should direct-list-linked_file", function()
    local setup = linked_file_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-linked_file", setup.live and "live" or "unit")
  ...
  it("should direct-load-linked_file", function()
    local setup = linked_file_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-linked_file", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function linked_file_direct_setup(mockres)
