...
describe("EpicDirect", function()
  it("should direct-list-epic", function()
    local setup = epic_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-epic", setup.live and "live" or "unit")
  ...
  it("should direct-load-epic", function()
    local setup = epic_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-epic", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function epic_direct_setup(mockres)
