...
describe("ObjectiveDirect", function()
  it("should direct-list-objective", function()
    local setup = objective_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-objective", setup.live and "live" or "unit")
  ...
  it("should direct-load-objective", function()
    local setup = objective_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-objective", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function objective_direct_setup(mockres)
