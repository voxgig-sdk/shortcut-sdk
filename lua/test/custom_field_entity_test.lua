...
describe("CustomFieldEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_CUSTOM_FIELD_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local custom_field_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.custom_field")))
    local custom_field_ref01_data = nil
    if #custom_field_ref01_data_raw > 0 then
      custom_field_ref01_data = helpers.to_map(custom_field_ref01_data_raw[1][2])
    end
    ...
    local custom_field_ref01_markdef_up0_name = "after_id"
...
function custom_field_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SHORTCUT_TEST_CUSTOM_FIELD_ENTID")
