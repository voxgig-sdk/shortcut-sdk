...
local ObjectifEntity = {}
ObjectifEntity.__index = ObjectifEntity


function ObjectifEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ObjectifEntity)
  self._name = "objectif"
  self._client = client
...
function ObjectifEntity:get_name()
  return self._name
end


function ObjectifEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ObjectifEntity.new(self._client, opts)
end
...
function ObjectifEntity:mark_deleted()
  self._deleted = true
end


function ObjectifEntity:deleted()
  return true == self._deleted
end


function ObjectifEntity:data_set(args)
  if args ~= nil then
...
function ObjectifEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ObjectifEntity:match_set(args)
  if args ~= nil then
...
function ObjectifEntity:match_get()
...
function ObjectifEntity:stream(action, args, callopts)
...
---@param reqmatch ObjectifRemoveMatch
---@param ctrl? table
---@return Objectif
---@return string? err
function ObjectifEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ObjectifEntity:_run_op(ctx, post_done)
  local utility = self._utility
