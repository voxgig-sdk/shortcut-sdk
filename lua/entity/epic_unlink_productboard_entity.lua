-- Shortcut SDK EpicUnlinkProductboard entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EpicUnlinkProductboardEntity = {}
EpicUnlinkProductboardEntity.__index = EpicUnlinkProductboardEntity


function EpicUnlinkProductboardEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EpicUnlinkProductboardEntity)
  self._name = "epic_unlink_productboard"
  self._client = client
...
function EpicUnlinkProductboardEntity:get_name()
  return self._name
end


function EpicUnlinkProductboardEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EpicUnlinkProductboardEntity.new(self._client, opts)
end
...
function EpicUnlinkProductboardEntity:mark_deleted()
  self._deleted = true
end


function EpicUnlinkProductboardEntity:deleted()
  return true == self._deleted
end


function EpicUnlinkProductboardEntity:data_set(args)
  if args ~= nil then
...
function EpicUnlinkProductboardEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EpicUnlinkProductboardEntity:match_set(args)
  if args ~= nil then
...
function EpicUnlinkProductboardEntity:match_get()
...
function EpicUnlinkProductboardEntity:stream(action, args, callopts)
...
---@param reqdata EpicUnlinkProductboardCreateData
---@param ctrl? table
---@return EpicUnlinkProductboard
---@return string? err
function EpicUnlinkProductboardEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EpicUnlinkProductboardEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return EpicUnlinkProductboardEntity
