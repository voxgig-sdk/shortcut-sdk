...
local StoryReactionEntity = {}
StoryReactionEntity.__index = StoryReactionEntity


function StoryReactionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StoryReactionEntity)
  self._name = "story_reaction"
  self._client = client
...
function StoryReactionEntity:get_name()
  return self._name
end


function StoryReactionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StoryReactionEntity.new(self._client, opts)
end
...
function StoryReactionEntity:mark_deleted()
  self._deleted = true
end


function StoryReactionEntity:deleted()
  return true == self._deleted
end


function StoryReactionEntity:data_set(args)
  if args ~= nil then
...
function StoryReactionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StoryReactionEntity:match_set(args)
  if args ~= nil then
...
function StoryReactionEntity:match_get()
...
function StoryReactionEntity:stream(action, args, callopts)
...
---@param reqdata StoryReactionCreateData
---@param ctrl? table
---@return StoryReaction
---@return string? err
function StoryReactionEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqmatch StoryReactionRemoveMatch
---@param ctrl? table
---@return StoryReaction
---@return string? err
function StoryReactionEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function StoryReactionEntity:_run_op(ctx, post_done)
  local utility = self._utility
