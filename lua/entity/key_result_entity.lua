...
local KeyResultEntity = {}
KeyResultEntity.__index = KeyResultEntity


function KeyResultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, KeyResultEntity)
  self._name = "key_result"
  self._client = client
...
function KeyResultEntity:get_name()
  return self._name
end


function KeyResultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return KeyResultEntity.new(self._client, opts)
end
...
function KeyResultEntity:mark_deleted()
  self._deleted = true
end


function KeyResultEntity:deleted()
  return true == self._deleted
end


function KeyResultEntity:data_set(args)
  if args ~= nil then
...
function KeyResultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function KeyResultEntity:match_set(args)
  if args ~= nil then
...
function KeyResultEntity:match_get()
...
function KeyResultEntity:stream(action, args, callopts)
...
---@param reqmatch KeyResultLoadMatch
---@param ctrl? table
---@return KeyResult
---@return string? err
function KeyResultEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata KeyResultUpdateData
---@param ctrl? table
---@return KeyResult
---@return string? err
function KeyResultEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function KeyResultEntity:_run_op(ctx, post_done)
  local utility = self._utility
