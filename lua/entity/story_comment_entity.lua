...
local StoryCommentEntity = {}
StoryCommentEntity.__index = StoryCommentEntity


function StoryCommentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StoryCommentEntity)
  self._name = "story_comment"
  self._client = client
...
function StoryCommentEntity:get_name()
  return self._name
end


function StoryCommentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StoryCommentEntity.new(self._client, opts)
end
...
function StoryCommentEntity:mark_deleted()
  self._deleted = true
end


function StoryCommentEntity:deleted()
  return true == self._deleted
end


function StoryCommentEntity:data_set(args)
  if args ~= nil then
...
function StoryCommentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StoryCommentEntity:match_set(args)
  if args ~= nil then
...
function StoryCommentEntity:match_get()
...
function StoryCommentEntity:stream(action, args, callopts)
...
---@param reqmatch StoryCommentLoadMatch
---@param ctrl? table
---@return StoryComment
---@return string? err
function StoryCommentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch StoryCommentListMatch
---@param ctrl? table
---@return StoryComment[]
---@return string? err
function StoryCommentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata StoryCommentCreateData
---@param ctrl? table
---@return StoryComment
---@return string? err
function StoryCommentEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata StoryCommentUpdateData
---@param ctrl? table
---@return StoryComment
---@return string? err
function StoryCommentEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function StoryCommentEntity:_run_op(ctx, post_done)
  local utility = self._utility
