...
local StorySlimEntity = {}
StorySlimEntity.__index = StorySlimEntity


function StorySlimEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StorySlimEntity)
  self._name = "story_slim"
  self._client = client
...
function StorySlimEntity:get_name()
  return self._name
end


function StorySlimEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StorySlimEntity.new(self._client, opts)
end
...
function StorySlimEntity:mark_deleted()
  self._deleted = true
end


function StorySlimEntity:deleted()
  return true == self._deleted
end


function StorySlimEntity:data_set(args)
  if args ~= nil then
...
function StorySlimEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StorySlimEntity:match_set(args)
  if args ~= nil then
...
function StorySlimEntity:match_get()
...
function StorySlimEntity:stream(action, args, callopts)
...
---@param reqdata StorySlimCreateData
---@param ctrl? table
---@return StorySlim
---@return string? err
function StorySlimEntity:create(reqdata, ctrl)
  local utility = self._utility
...
---@param reqdata StorySlimUpdateData
---@param ctrl? table
---@return StorySlim
---@return string? err
function StorySlimEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function StorySlimEntity:_run_op(ctx, post_done)
  local utility = self._utility
