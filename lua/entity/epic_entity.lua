...
local EpicEntity = {}
EpicEntity.__index = EpicEntity


function EpicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EpicEntity)
  self._name = "epic"
  self._client = client
...
function EpicEntity:get_name()
  return self._name
end


function EpicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EpicEntity.new(self._client, opts)
end
...
function EpicEntity:mark_deleted()
  self._deleted = true
end


function EpicEntity:deleted()
  return true == self._deleted
end


function EpicEntity:data_set(args)
  if args ~= nil then
...
function EpicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EpicEntity:match_set(args)
  if args ~= nil then
...
function EpicEntity:stream(action, args, callopts)
...
---@param reqmatch EpicLoadMatch
---@param ctrl? table
---@return Epic
---@return string? err
function EpicEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch EpicListMatch
---@param ctrl? table
---@return Epic[]
---@return string? err
function EpicEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata EpicCreateData
---@param ctrl? table
---@return Epic
---@return string? err
function EpicEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata EpicUpdateData
---@param ctrl? table
---@return Epic
---@return string? err
function EpicEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch EpicRemoveMatch
---@param ctrl? table
---@return Epic
---@return string? err
function EpicEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EpicEntity:_run_op(ctx, post_done)
  local utility = self._utility
