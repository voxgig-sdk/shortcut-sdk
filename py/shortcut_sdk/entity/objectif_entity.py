...
class ObjectifEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ObjectifEntity(self._client, opts)
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Objectif:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Objectif().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
