...
class StorySlimEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return StorySlimEntity(self._client, opts)
    ...
    def create(self, reqdata: StorySlimCreateData, ctrl=None) -> StorySlim:
        utility = self._utility
    ...
    def update(self, reqdata: StorySlimUpdateData, ctrl=None) -> StorySlim:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
