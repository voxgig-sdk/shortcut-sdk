...
class StoryReactionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return StoryReactionEntity(self._client, opts)
    ...
    def data_get(self) -> StoryReaction:
    ...
    def match_get(self) -> StoryReaction:
    ...
    def create(self, reqdata: StoryReactionCreateData, ctrl=None) -> StoryReaction:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> StoryReaction:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.StoryReaction().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
