...
class DocSlimEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DocSlimEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DocSlim]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DocSlim().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: DocSlimCreateData, ctrl=None) -> DocSlim:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
