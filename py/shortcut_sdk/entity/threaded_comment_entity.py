...
from shortcut_sdk.shortcut_types import (
    ThreadedComment,
...
class ThreadedCommentEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ThreadedCommentEntity(self._client, opts)
    ...
    def data_get(self) -> ThreadedComment:
    ...
    def match_get(self) -> ThreadedComment:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ThreadedComment:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ThreadedComment().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ThreadedComment]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ThreadedComment().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def create(self, reqdata: ThreadedCommentCreateData, ctrl=None) -> ThreadedComment:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: ThreadedCommentUpdateData, ctrl=None) -> ThreadedComment:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ThreadedComment:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ThreadedComment().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
