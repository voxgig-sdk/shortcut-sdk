...
class TestThreadedCommentDirect:

    def test_should_direct_list_threaded_comment(self):
        setup = _threaded_comment_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-threaded_comment", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["epic01"]:
    ...
    def test_should_direct_load_threaded_comment(self):
        setup = _threaded_comment_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-threaded_comment", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _threaded_comment_direct_setup(mockres):
