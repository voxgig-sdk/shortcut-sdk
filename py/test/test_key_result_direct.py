...
class TestKeyResultDirect:

    def test_should_direct_load_key_result(self):
        setup = _key_result_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-key_result", "live" if setup["live"] else "unit")
        if _skip:
...
def _key_result_direct_setup(mockres):
