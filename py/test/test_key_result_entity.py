...
class TestKeyResultEntity:

    def test_should_create_instance(self):
        testsdk = ShortcutSDK.test(None, None)
        ent = testsdk.KeyResult(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _key_result_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["update", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "key_result." + _op, "live" if _live else "unit")
            if _skip:
...
def _key_result_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/key_result/KeyResultTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHORTCUT_TEST_KEY_RESULT_ENTID"))
