...
class TestLinkedFileEntity:

    def test_should_create_instance(self):
        testsdk = ShortcutSDK.test(None, None)
        ent = testsdk.LinkedFile(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.LinkedFile(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.LinkedFile(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _linked_file_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "linked_file." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHORTCUT_TEST_LINKED_FILE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        linked_file_ref01_ent = client.LinkedFile(None)
        ...
        linked_file_ref01_list_result = linked_file_ref01_ent.list(linked_file_ref01_match, None)
        assert isinstance(linked_file_ref01_list_result, list)
        ...
        linked_file_ref01_ent.remove(linked_file_ref01_match_rm0, None)

        # LIST
        linked_file_ref01_match_rt0 = {}

        linked_file_ref01_list_rt0_result = linked_file_ref01_ent.list(linked_file_ref01_match_rt0, None)
        assert isinstance(linked_file_ref01_list_rt0_result, list)
...
def _linked_file_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/linked_file/LinkedFileTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "SHORTCUT_TEST_LINKED_FILE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHORTCUT_TEST_LINKED_FILE_ENTID"))
