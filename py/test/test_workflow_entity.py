...
class TestWorkflowEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Workflow(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Workflow(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHORTCUT_TEST_WORKFLOW_ENTID JSON to run live")
        client = setup["client"]
        ...
        workflow_ref01_list_result = workflow_ref01_ent.list(workflow_ref01_match, None)
        assert isinstance(workflow_ref01_list_result, list)
...
def _workflow_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHORTCUT_TEST_WORKFLOW_ENTID"))
