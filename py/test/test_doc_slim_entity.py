...
class TestDocSlimEntity:

    def test_should_create_instance(self):
        testsdk = ShortcutSDK.test(None, None)
        ent = testsdk.DocSlim(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.DocSlim(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.DocSlim(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _doc_slim_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "doc_slim." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHORTCUT_TEST_DOC_SLIM_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        doc_slim_ref01_ent = client.DocSlim(None)
        ...
        doc_slim_ref01_list_result = doc_slim_ref01_ent.list(doc_slim_ref01_match, None)
        assert isinstance(doc_slim_ref01_list_result, list)
...
def _doc_slim_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/doc_slim/DocSlimTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHORTCUT_TEST_DOC_SLIM_ENTID"))
