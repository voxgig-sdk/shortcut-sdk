...
class TestThreadedCommentEntity:

    def test_should_create_instance(self):
        testsdk = ShortcutSDK.test(None, None)
        ent = testsdk.ThreadedComment(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.ThreadedComment(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.ThreadedComment(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _threaded_comment_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "threaded_comment." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHORTCUT_TEST_THREADED_COMMENT_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        threaded_comment_ref01_ent = client.ThreadedComment(None)
        ...
        threaded_comment_ref01_list_result = threaded_comment_ref01_ent.list(threaded_comment_ref01_match, None)
        assert isinstance(threaded_comment_ref01_list_result, list)
        ...
        threaded_comment_ref01_ent.remove(threaded_comment_ref01_match_rm0, None)
        ...
        threaded_comment_ref01_list_rt0_result = threaded_comment_ref01_ent.list(threaded_comment_ref01_match_rt0, None)
        assert isinstance(threaded_comment_ref01_list_rt0_result, list)
...
def _threaded_comment_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/threaded_comment/ThreadedCommentTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHORTCUT_TEST_THREADED_COMMENT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)
    if idmap_resolved.get("epic_id") is None:
        idmap_resolved["epic_id"] = idmap_resolved.get("epic01")
