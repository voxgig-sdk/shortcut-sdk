...
class ShortcutTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ShortcutTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ShortcutTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ShortcutTestRunner.getenv("SHORTCUT_TEST_LIVE")
        override = ShortcutTestRunner.getenv("SHORTCUT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ShortcutTestRunner.getenv(key)
        ...
        explain = ShortcutTestRunner.getenv("SHORTCUT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["SHORTCUT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ShortcutTestRunner._test_control is not None:
            return ShortcutTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ShortcutTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ShortcutTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ShortcutTestRunner.load_test_control()
...
def load_env_local():
    ShortcutTestRunner.load_env_local()


def env_override(m):
    return ShortcutTestRunner.env_override(m)


def entity_data(v):
    return ShortcutTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ShortcutTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ShortcutTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ShortcutTestRunner.load_test_control()


def live_client_options():
    return ShortcutTestRunner.live_client_options()


def live_delay_ms():
    return ShortcutTestRunner.live_delay_ms()
