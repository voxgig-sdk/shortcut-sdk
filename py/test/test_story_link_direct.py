...
class TestStoryLinkDirect:

    def test_should_direct_load_story_link(self):
        setup = _story_link_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-story_link", "live" if setup["live"] else "unit")
        if _skip:
...
def _story_link_direct_setup(mockres):
