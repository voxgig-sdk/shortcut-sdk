...
class TestDocSlimDirect:

    def test_should_direct_list_doc_slim(self):
        setup = _doc_slim_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-doc_slim", "live" if setup["live"] else "unit")
        if _skip:
...
def _doc_slim_direct_setup(mockres):
