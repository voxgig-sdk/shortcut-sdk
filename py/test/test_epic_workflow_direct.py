...
class TestEpicWorkflowDirect:

    def test_should_direct_list_epic_workflow(self):
        setup = _epic_workflow_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-epic_workflow", "live" if setup["live"] else "unit")
        if _skip:
...
def _epic_workflow_direct_setup(mockres):
