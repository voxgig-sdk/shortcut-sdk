...
class ThreadedCommentEntity
  ...
  def make
    opts = @_entopts.dup
    ThreadedCommentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ShortcutHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ThreadedComment, Hash] the current ThreadedComment data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ShortcutHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ThreadedComment fields)
  def match_get
  ...
  #
  # @param reqmatch [ThreadedCommentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ThreadedComment.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ThreadedComment, Hash] the loaded ThreadedComment; raises ShortcutError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ShortcutHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [ThreadedCommentListMatch, Hash, nil] match filter (any subset of
  #   ThreadedComment fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ThreadedComment>, Array] the matching ThreadedComment items; raises ShortcutError on failure
  ...
  #
  # @param reqdata [ThreadedCommentCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [ThreadedComment, Hash] the created ThreadedComment; raises ShortcutError on failure
  ...
  #
  # @param reqdata [ThreadedCommentUpdateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [ThreadedComment, Hash] the updated ThreadedComment; raises ShortcutError on failure
  ...
  #
  # @param reqmatch [ThreadedCommentRemoveMatch, Hash, nil] match criteria (id/query fields)
  # @param ctrl [Object, nil] optional per-call control
  # @return [ThreadedComment, Hash] the removed ThreadedComment; raises ShortcutError on failure
