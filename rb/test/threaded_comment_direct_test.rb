...
class ThreadedCommentDirectTest < Minitest::Test
  def test_direct_list_threaded_comment
    setup = threaded_comment_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-threaded_comment", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["epic01"].each do |_live_key|
  ...
  def test_direct_load_threaded_comment
    setup = threaded_comment_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-threaded_comment", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def threaded_comment_direct_setup(mockres)
