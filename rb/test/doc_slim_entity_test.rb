...
class DocSlimEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.DocSlim(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.DocSlim(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = doc_slim_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "doc_slim." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_DOC_SLIM_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    doc_slim_ref01_ent = client.DocSlim(nil)
    ...
    doc_slim_ref01_list_result = doc_slim_ref01_ent.list(doc_slim_ref01_match, nil)
    assert doc_slim_ref01_list_result.is_a?(Array)
...
def doc_slim_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "doc_slim", "DocSlimTestData.json")
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_DOC_SLIM_ENTID"]
