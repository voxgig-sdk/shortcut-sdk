...
class LabelEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_LABEL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    label_ref01_ent.remove(label_ref01_match_rm0, nil)

    # LIST
    label_ref01_match_rt0 = {}

    label_ref01_list_rt0_result = label_ref01_ent.list(label_ref01_match_rt0, nil)
    assert label_ref01_list_rt0_result.is_a?(Array)
...
def label_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_LABEL_ENTID"]
