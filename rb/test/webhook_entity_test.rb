...
class WebhookEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_WEBHOOK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    assert !webhook_ref01_data_dt0_loaded.nil?
...
def webhook_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_WEBHOOK_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["SHORTCUT_TEST_WEBHOOK_ENTID"])
