...
class StoryLinkEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = story_link_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "update", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "story_link." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_STORY_LINK_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    story_link_ref01_ent = client.StoryLink(nil)
    ...
    story_link_ref01_ent.remove(story_link_ref01_match_rm0, nil)

  end
end

def story_link_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "story_link", "StoryLinkTestData.json")
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_STORY_LINK_ENTID"]
