...
class ThreadedCommentEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.ThreadedComment(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.ThreadedComment(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = threaded_comment_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "threaded_comment." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_THREADED_COMMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    threaded_comment_ref01_ent = client.ThreadedComment(nil)
    ...
    threaded_comment_ref01_list_result = threaded_comment_ref01_ent.list(threaded_comment_ref01_match, nil)
    assert threaded_comment_ref01_list_result.is_a?(Array)
    ...
    threaded_comment_ref01_ent.remove(threaded_comment_ref01_match_rm0, nil)
    ...
    threaded_comment_ref01_list_rt0_result = threaded_comment_ref01_ent.list(threaded_comment_ref01_match_rt0, nil)
    assert threaded_comment_ref01_list_rt0_result.is_a?(Array)
...
def threaded_comment_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "threaded_comment", "ThreadedCommentTestData.json")
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_THREADED_COMMENT_ENTID"]
