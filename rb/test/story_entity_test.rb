...
class StoryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_STORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    story_ref01_ent.remove(story_ref01_match_rm0, nil)

    # LIST
    story_ref01_match_rt0 = {}

    story_ref01_list_rt0_result = story_ref01_ent.list(story_ref01_match_rt0, nil)
    assert story_ref01_list_rt0_result.is_a?(Array)
...
def story_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_STORY_ENTID"]
