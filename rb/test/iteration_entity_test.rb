...
class IterationEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Iteration(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Iteration(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = iteration_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load", "remove"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "iteration." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_ITERATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    iteration_ref01_ent = client.Iteration(nil)
    ...
    iteration_ref01_list_result = iteration_ref01_ent.list(iteration_ref01_match, nil)
    assert iteration_ref01_list_result.is_a?(Array)
    ...
    iteration_ref01_ent.remove(iteration_ref01_match_rm0, nil)

    # LIST
    iteration_ref01_match_rt0 = {}

    iteration_ref01_list_rt0_result = iteration_ref01_ent.list(iteration_ref01_match_rt0, nil)
    assert iteration_ref01_list_rt0_result.is_a?(Array)
...
def iteration_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "iteration", "IterationTestData.json")
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_ITERATION_ENTID"]
