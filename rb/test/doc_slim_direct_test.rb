...
class DocSlimDirectTest < Minitest::Test
  def test_direct_list_doc_slim
    setup = doc_slim_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-doc_slim", setup[:live] ? "live" : "unit")
    if _should_skip
...
def doc_slim_direct_setup(mockres)
