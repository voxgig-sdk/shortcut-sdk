...
class CommentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_COMMENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    comment_ref01_data = nil
    if comment_ref01_data_raw.length > 0
...
def comment_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_COMMENT_ENTID"]
