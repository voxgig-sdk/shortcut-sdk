...
class RepositoryEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Repository(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Repository(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = repository_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "repository." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_REPOSITORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    repository_ref01_data = nil
    if repository_ref01_data_raw.length > 0
      repository_ref01_data = Helpers.to_map(repository_ref01_data_raw[0][1])
    end

    # LIST
    repository_ref01_ent = client.Repository(nil)
    repository_ref01_match = {}

    repository_ref01_list_result = repository_ref01_ent.list(repository_ref01_match, nil)
    assert repository_ref01_list_result.is_a?(Array)
...
def repository_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "repository", "RepositoryTestData.json")
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_REPOSITORY_ENTID"]
