...
class ObjectiveEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Objective(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Objective(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = objective_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "list", "update", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "objective." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTCUT_TEST_OBJECTIVE_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    objective_ref01_ent = client.Objective(nil)
    ...
    objective_ref01_list_result = objective_ref01_ent.list(objective_ref01_match, nil)
    assert objective_ref01_list_result.is_a?(Array)
...
def objective_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "objective", "ObjectiveTestData.json")
  ...
  entid_env_raw = ENV["SHORTCUT_TEST_OBJECTIVE_ENTID"]
