...
module ShortcutUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ShortcutResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ShortcutResponse.new(fetched)
    else
